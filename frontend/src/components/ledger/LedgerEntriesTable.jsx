...
function LedgerEntriesTable({ ledgerEntries, categories }) {
    ...
    const accountOptions = useMemo(() => {
        const unique = [...new Set(entries.map((entry) => entry.account).filter(Boolean))]
    ...
    const filteredEntries = useMemo(() => {
        ...
        return entries.filter((entry) => {
            ...
            if (normalizedSearch) {
                const target = `${entry.description} ${entry.account} ${entryCategory}`.toLowerCase()
    ...
    }, [entries, searchTerm, onlyUncategorized, appliedFilters])
